...
public class NoteActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                CharSequence title_con=editTitle.getText();
                CharSequence content = editText.getText();

                if(TextUtils.isEmpty(title_con)){
                    Toast.makeText(NoteActivity.this,
                            "No Title", Toast.LENGTH_SHORT).show();
                }
    ...
    private boolean saveNote2Database(String content, String title_con,Priority priority) {
        if (database == null || TextUtils.isEmpty(content)) {
            return false;
        }
        ContentValues values = new ContentValues();
        values.put(TodoNote.COLUMN_CONTENT, content);
        values.put(TodoNote.COLUMN_TITLE,title_con);
